...
res_train = (res_train-res_train.min())/(res_train.max()-res_train.min())

#Applying linear regression on the data

gnb = GaussianNB()
model = gnb.fit(train_data, res_train)
...
print("R2 Score is", model.score(testdata, res_test))

print("Mean Squared Error is ", mean_squared_error(model.predict(testdata), res_test))
