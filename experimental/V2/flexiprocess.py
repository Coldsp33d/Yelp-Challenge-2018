...
if args['withCluster']:
    ...
    outFile_name += '_withCluster_{args["clusterScheme"].title()}'
    ...
    cat_df.to_csv('yelp_dataset/catmap_{args["clusterScheme"].title()}.csv')
...
if args['withTrainTestSplit']:
    ...
    train.to_csv('yelp_dataset/{outFile_name}_train.csv')
    test.to_csv('yelp_dataset/{outFile_name}_test.csv')
else:
    business_json_df.to_csv('yelp_dataset/{outFile_name}.csv')
